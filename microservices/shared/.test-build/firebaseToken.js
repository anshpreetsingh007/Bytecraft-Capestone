...
exports.resetSigningKeyCache = resetSigningKeyCache;
/**
...
async function fetchSigningKeys() {
    ...
    for (const jwk of body.keys ?? []) {
        if (!jwk.kid || jwk.kty !== 'RSA' || !jwk.n || !jwk.e)
            continue;
        try {
            keys.set(jwk.kid, (0, node_crypto_1.createPublicKey)({ key: { kty: 'RSA', n: jwk.n, e: jwk.e }, format: 'jwk' }));
        }
        catch (error) {
            logger_1.logger.warn('skipped an unusable Google signing key', { kid: jwk.kid, error });
        }
    }
    if (keys.size === 0)
...
async function getSigningKey(kid, allowRefresh = true) {
    ...
    throw (0, errors_1.unauthorized)('Your session could not be verified, please sign in again');
}
...
function decodeJsonSegment(segment, label) {
    try {
        ...
        throw (0, errors_1.unauthorized)(`Your session token has a malformed ${label}`);
    }
}
async function verifyFirebaseIdToken(token) {
    const segments = token.split('.');
    if (segments.length !== 3)
        throw (0, errors_1.unauthorized)('Your session token is malformed');
    ...
    if (header.alg !== 'RS256') {
        ...
        throw (0, errors_1.unauthorized)('Your session token uses an unsupported signing algorithm');
    }
    if (!header.kid)
        throw (0, errors_1.unauthorized)('Your session token is missing a key id');
    ...
    if (!verifier.verify(key, decodeBase64Url(encodedSignature))) {
        throw (0, errors_1.unauthorized)('Your session token signature is not valid');
    }
    const payload = decodeJsonSegment(encodedPayload, 'payload');
    const projectId = firebaseProjectId();
    const nowSeconds = Math.floor(Date.now() / 1000);
    if (payload.aud !== projectId) {
        throw (0, errors_1.unauthorized)('Your session token was issued for a different application');
    }
    if (payload.iss !== `https://securetoken.google.com/${projectId}`) {
        throw (0, errors_1.unauthorized)('Your session token has an unexpected issuer');
    }
    if (typeof payload.sub !== 'string' || payload.sub.length === 0 || payload.sub.length > 128) {
        throw (0, errors_1.unauthorized)('Your session token has no valid subject');
    }
    if (typeof payload.exp !== 'number' || payload.exp + CLOCK_SKEW_SECONDS < nowSeconds) {
        throw (0, errors_1.unauthorized)('Your session has expired, please sign in again');
    }
    if (typeof payload.iat !== 'number' || payload.iat - CLOCK_SKEW_SECONDS > nowSeconds) {
        throw (0, errors_1.unauthorized)('Your session token was issued in the future');
    }
    if (typeof payload.auth_time === 'number' && payload.auth_time - CLOCK_SKEW_SECONDS > nowSeconds) {
        throw (0, errors_1.unauthorized)('Your session token reports a future sign-in time');
    }
    return {
        uid: payload.sub,
